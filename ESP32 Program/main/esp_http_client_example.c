...
#define WEB_SERVER "stt.api.cloud.yandex.net"
#define WEB_PORT "443"
#define FOLDER_ID "b1g9dniheg5j2s3219dt"
#define IAM_TOKEN "t1.9euelZqQj5ianpmRkp6NnsqMjpOMj-3rnpWayJ2XxpiajY-LjZ6Sm5nHy5Tl8_dJanxU-e8DWkBd_N3z9wkZelT57wNaQF38zef1656Vmo2UlMmJjovLipTHl8aTnYvL7_zF656Vmo2UlMmJjovLipTHl8aTnYvL.HMCp-qt73L1dmwAwzp05Db7jPawBFnaPGgygzaFE3z3wxBwm4QnLHxfOyAsvP-JsXuqZqyoGdtg2WZBNait1DA"
#define WEB_PATH "https://stt.api.cloud.yandex.net/speech/v1/stt:recognize?topic=general&lang=en-US&format=lpcm&sampleRateHertz=8000&folderId="FOLDER_ID
...
#define BUF_SIZE_16_BIT  12000

static const char *TAG = "HTTP_CLIENT";
static const char *TAG_UART = "UART_TEST";
...
uint16_t dataFromADC[BUF_SIZE_16_BIT];
...
static void http_rest_with_hostname_path(void)
{
    ...
    // POST
    esp_http_client_set_header(client, "Authorization", "Bearer "IAM_TOKEN);
    esp_http_client_set_post_field(client, (char *)data, BUF_SIZE);
    esp_err_t err = esp_http_client_perform(client);
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "HTTP POST Status = %d, content_length = %"PRIu64,
                esp_http_client_get_status_code(client),
...
static void https_with_url(void)
{
   ...
   // esp_http_client_read_response(client, dataReceived, (int)esp_http_client_get_content_length(client));
    esp_http_client_read(client, dataReceived, (int)esp_http_client_get_content_length(client));
    dataReceived = getCharData(client);
    ESP_LOGI(TAG, "DataReceived: %s", dataReceived);
...
static void https_with_hostname_path(void)
{
    esp_http_client_config_t config = {
		...
		.cert_len = howsmyssl_com_root_cert_pem_end - howsmyssl_com_root_cert_pem_start,
...
static void http_test_task(void *pvParameters)
{
	...
	xQueueSemaphoreTake(UART_Semaphore, portMAX_DELAY);

    ESP_LOGI(TAG, "DataFromADC");
...
static void uart_echo_task(void *arg)
{
    ...
    uint16_t i = 0;
    //uint16_t count = 0;
    //static char myStr[REQUEST_SIZE] = {};
    ESP_LOGI(TAG_UART, "%s", "Привет");
    while (1) {
        // Read data from the UART

    	int len = uart_read_bytes(ECHO_UART_PORT_NUM, data, (BUF_SIZE), portMAX_DELAY);

    	//memset(myStr, '\0', sizeof myStr);
    	//memset(Result_To_Request, '\0', sizeof Result_To_Request);
       ...
       // uart_write_bytes(ECHO_UART_PORT_NUM, (const char *) data, len);
        ...
        if (len) {
            //data[len] = '\0';
            ESP_LOGI(TAG_UART, "Recv data: ");

            ESP_LOGI(TAG_UART, "Data");
          ...
          // }
            ...
            {
            	dataFromADC[i] = data[i*2] | (data[(i*2) + 1] << 8);
            }



            ESP_LOGI(TAG_UART, "len: %d", len);
            //ESP_LOGI(TAG_UART, "data: %d")

            //xSemaphoreGive(UART_Semaphore);
           //

            //ConfigJSONToRequest(data);
            //strcat(myStr, RESULT_TO_JSON_1);
            //strcat(myStr, Result_To_Request);
            //strcat(myStr, RESULT_TO_JSON_2);
            //ESP_LOGI(TAG_UART, "Result_To_Request: %s", Result_To_Request);
            //i = 0;
            //printf("Result_To_Request:\n");
            //while(Result_To_Request[i] != '\0')
            //{
            //	 printf("%c", Result_To_Request[i]);
            //	 i++;
            //}


            //ESP_LOGI(TAG_UART, "myStr: %s", myStr);
          //  i = 0;
            //count = 0;
        	//while(myStr[i] != '\0')
        	//{
            ...
            //xQueueSend(UART_Queue, dataFromADC, portMAX_DELAY);
            xSemaphoreGive(UART_Semaphore);
...
void app_main(void)
{
    ...
    http_test_task(NULL);
#else
    UART_Semaphore =  xSemaphoreCreateCounting(10, 0);
    xTaskCreate(&http_test_task, "http_test_task", 8192, NULL, 5, NULL);
    xTaskCreate(&uart_echo_task, "uart_echo_task", ECHO_TASK_STACK_SIZE, NULL, 5, NULL);
#endif
}
